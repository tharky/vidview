...
bool DecoderWorker::openCodec(
    ...
    if (!codecContext) {
        errorMessage = "Could not allocate codec context.";
        return false;
    }

    int result = avcodec_parameters_to_context(codecContext, stream->codecpar);
    if (result < 0) {
        errorMessage = "Could not copy codec parameters: " + ffmpegError(result);
    ...
    if (result < 0) {
        errorMessage = "Could not open decoder: " + ffmpegError(result);
        avcodec_free_context(&codecContext);
        return false;
    }
...
void DecoderWorker::openFile(const QString& path, quint64 generation) {
    closeFile();
    ...
    if (result < 0) {
        emit opened(
            false,
            "Could not open file: " + ffmpegError(result),
            0.0, 0.0, 0, 0, false, generation
        );
        return;
    }
    ...
    QString errorMessage;
    if (!openCodec(videoStreamIndex_, videoCodecContext_, errorMessage)) {
        emit opened(
            false,
            errorMessage,
            0.0, 0.0, 0, 0, false, generation
    ...
    width_ = videoCodecContext_->width;
    height_ = videoCodecContext_->height;

    AVStream* videoStream = formatContext_->streams[videoStreamIndex_];
    const AVRational guessedFrameRate =
        av_guess_frame_rate(formatContext_, videoStream, nullptr);

    if (guessedFrameRate.num > 0 && guessedFrameRate.den > 0) {
        fps_ = av_q2d(guessedFrameRate);
    }
    ...
    if (formatContext_->duration != AV_NOPTS_VALUE) {
        duration_ =
            static_cast<double>(formatContext_->duration) / AV_TIME_BASE;
    }
    ...
    if (audioStreamIndex_ >= 0) {
        ...
        if (openCodec(audioStreamIndex_, audioCodecContext_, audioError)) {
...
double DecoderWorker::timestampForFrame(
    ...
    const AVStream* stream = formatContext_->streams[streamIndex];
...
QImage DecoderWorker::convertVideoFrame() {
    ...
    const int sourceWidth = videoFrame_->width;
    const int sourceHeight = videoFrame_->height;

    const double scale = std::min({
        1.0,
        static_cast<double>(MaxPreviewWidth) / sourceWidth,
        static_cast<double>(MaxPreviewHeight) / sourceHeight
    });
...
void DecoderWorker::processVideoFrames(
    ...
    while (true) {
        if (!isDesiredGeneration(generation)) return;

        const int result =
            avcodec_receive_frame(videoCodecContext_, videoFrame_);

        if (result == AVERROR(EAGAIN) || result == AVERROR_EOF) return;
        if (result < 0) return;

        const double timestamp =
            timestampForFrame(videoFrame_, videoStreamIndex_);

        if (
            discardVideoBefore_ >= 0.0 &&
            timestamp >= 0.0 &&
            timestamp + 0.000001 < discardVideoBefore_
        ) {
        ...
        QImage image = convertVideoFrame();
        if (!image.isNull()) {
            emit videoFrameReady(
                std::move(image),
                std::max(timestamp, 0.0),
                generation
            );
            ++producedFrames;
        }
...
void DecoderWorker::processAudioFrames(quint64 generation) {
    if (!audioCodecContext_ || !swrContext_) return;

    while (true) {
        if (!isDesiredGeneration(generation)) return;

        const int result =
            avcodec_receive_frame(audioCodecContext_, audioFrame_);

        if (result == AVERROR(EAGAIN) || result == AVERROR_EOF) return;
        if (result < 0) return;

        const double timestamp =
            timestampForFrame(audioFrame_, audioStreamIndex_);

        if (
            discardAudioBefore_ >= 0.0 &&
            timestamp >= 0.0 &&
            timestamp + 0.000001 < discardAudioBefore_
        ) {
        ...
        QByteArray pcm;
        pcm.resize(outputSamples * AudioChannels * bytesPerSample);

        uint8_t* outputData[1] = {
            reinterpret_cast<uint8_t*>(pcm.data())
        };
        const uint8_t* const* inputData = audioFrame_->extended_data;
...
void DecoderWorker::decodeBatch(
    ...
    if (
        ...
        !isDesiredGeneration(generation) ||
        !formatContext_ ||
        eof_
    ) {
    ...
    while (producedFrames < maxVideoFrames) {
        if (!isDesiredGeneration(generation)) {
            emit batchFinished(generation);
            return;
        }

        int result = av_read_frame(formatContext_, packet_);

        if (result < 0) {
            avcodec_send_packet(videoCodecContext_, nullptr);
            processVideoFrames(producedFrames, generation);

            if (audioCodecContext_) {
                avcodec_send_packet(audioCodecContext_, nullptr);
                processAudioFrames(generation);
            }

            eof_ = true;
            emit endOfStream(generation);
            break;
        }
        ...
        if (packet_->stream_index == videoStreamIndex_) {
            ...
            audioCodecContext_ &&
            packet_->stream_index == audioStreamIndex_
        ) {
...
void DecoderWorker::resetResampler() {
    if (!swrContext_) return;
    swr_close(swrContext_);
    swr_init(swrContext_);
}

void DecoderWorker::seek(double seconds, quint64 generation) {
    ...
    if (result < 0) {
        emit seekFinished(seconds, generation);
        return;
    }
